...
func mustSource(t *testing.T, name, data string) *so.Source {
	t.Helper()
	s, err := so.New(name, []byte(data))
	if err != nil {
		t.Fatalf("so.New(%q): %v", name, err)
	}
	return s
}

// stageBuild populates dir as a finished build for src: a bin plus a manifest
// whose checksum matches, so it satisfies CachedBin once it lives at AppDir.
func stageBuild(t *testing.T, dir string, src *so.Source) {
	t.Helper()
	binPath := filepath.Join(dir, "bin", binName)
	if err := os.MkdirAll(filepath.Dir(binPath), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(binPath, []byte("bin:"+string(src.AppKey())), 0o700); err != nil { //nolint:gosec // test fixture
		t.Fatal(err)
	}
...
func TestAppDirNamespacedBySchema(t *testing.T) {
	...
	if rel, err := filepath.Rel(c.Path(), dir); err != nil || filepath.Dir(rel) != want {
		t.Fatalf("AppDir = %q, want under %q", dir, want)
	}
	if !strings.HasSuffix(filepath.Base(dir), "-app") {
		t.Fatalf("AppDir base %q missing slug suffix", filepath.Base(dir))
	}
}
...
func TestCachedBinGate(t *testing.T) {
	...
	if bin, ok := c.CachedBin(src); !ok || bin != c.AppBinFile(src) {
		t.Fatalf("valid entry: bin=%q ok=%v", bin, ok)
	}

	// Checksum mismatch → miss (guards post-publish corruption).
	if err := os.WriteFile(c.AppBinFile(src), []byte("tampered"), 0o700); err != nil { //nolint:gosec // test fixture
		t.Fatal(err)
	}
	if _, ok := c.CachedBin(src); ok {
...
func TestPublish(t *testing.T) {
	...
	t.Run("valid dest yields to peer", func(t *testing.T) {
		c := CacheRoot(t.TempDir())
		src := mustSource(t, "app.gorn", "body")
		stageBuild(t, c.AppDir(src), src) // peer already published
